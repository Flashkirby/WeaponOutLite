...
namespace WeaponOutLite.Content.DrawItemPose
{
    ...
    /// </summary>
    public class FloatingOrbitAntiClockwiseSlow : IDrawItemPose
    {
        public virtual int GetID() => DrawItemPoseID.FloatingOrbitAntiClockwiseSlow;

        public virtual short DrawDepth(Player p, Item i, int timer) {
            if (-MathF.Cos(Main.GlobalTimeWrappedHourly) < 0f) {
        ...
        public virtual DrawData CalculateDrawData(DrawData data, Player p, float height, float width, int bodyFrame, int timer) {
            ...
            Vector2 orbit = new Vector2(
                -MathF.Sin(Main.GlobalTimeWrappedHourly) * p.direction,
                -MathF.Cos(Main.GlobalTimeWrappedHourly) / 5f);
